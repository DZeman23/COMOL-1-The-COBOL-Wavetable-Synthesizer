      ...
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * Load a signed 16-bit PCM RAW file (2048 samples, 44100 Hz)
      * into the LFO wavetable when waveform type 8 is selected.
      * Both LFO1 and LFO2 share this same source file.
           SELECT LFO-FILE ASSIGN TO
           ""path\to\DetunedSaw.raw"
           ORGANIZATION IS SEQUENTIAL
           ACCESS MODE IS SEQUENTIAL.
       DATA DIVISION.
       FILE SECTION.
      ...
      * Two-byte record for reading the LFO user RAW waveform file.
      * Treated as a single character at a time (byte-by-byte read).
       FD  LFO-FILE.
      ...
      * /\       MENU & USER INPUT        /\
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * NOTE: WAVE-SOURCE-CHOICE left as plain PIC (not COMP) so ACCEPT
      *       works correctly on all compilers.
      ...
      * TVF-DEPTH kept as signed integer so the user can enter -100 to 100
      ...
      * Changed from COMP-3 to COMP-5 for binary integer performance
      ...
      * Reduced integer part from S9(12) to S9(5) - matches hardware limit
      ...
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * Each LFO gets its own 2048-point floating-point wavetable.
      * Populated at init time based on the waveform type selected.
      * Range is -1.0 to +1.0 for all waveforms.
       01  LFO1-WAVE-TABLE.
      ...
      * TVA-DEPTH: -100 to +100. Scales LFO into amplitude.
      * TVF-DEPTH: -100 to +100. Added (in knob units) to CURRENT-KNOB.
      * PTCH-DEPTH: in units of 0.1 semitone. +/- 120 = +/- 12 semitones.
      * PHASE-OFFS: 0-359 degrees. Sets starting phase of waveform.
      * FM-DEPTH: LFO1 rate modulates LFO2 rate. -100 to +100 percent.
      * FADE-SEC: positive = fade in, negative = fade out (seconds).
       01  LFO1-PARAMS.
      ...
      * LFO2 has no FM-DEPTH (it is the target of LFO1 FM)
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * /\     LFO 1 RUNTIME STATE       /\
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * All COMP-5 for maximum arithmetic speed in the sample loop.
      * PHASE is in wavetable index units (0.0 to 2047.999...).
      * STEP is how far phase advances per sample.
      * DELAY-SMPL is the total delay in samples.
      * DELAY-CNT  counts down from DELAY-SMPL to 0.
      * FADE-TOTAL is total fade duration in samples.
      * FADE-CNT   counts how many samples have elapsed in fade.
      * FADE-AMT   is the current fade multiplier (0.0 to 1.0).
      * FADE-STEP  is added to FADE-AMT each sample.
      * SH-HELD    is the last held random value for S+H mode.
      * SH-PERIOD  is the number of samples between S+H triggers.
      * SH-CNT     counts samples since the last S+H trigger.
      * PREV-IDX   tracks previous table integer index for edge detect.
      ...
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * LFO1-VALUE / LFO2-VALUE: final per-sample output (-1.0 to 1.0)
      *   after waveform lookup, fade, and offset have been applied.
      * LFO-TVA-OUT: combined amp modulation. Applied post-filter.
      * LFO-TVF-OUT: combined cutoff offset. Added to CURRENT-KNOB.
      * LFO-PTCH-OUT: combined pitch shift (semitones). Added to phase.
      * LFO-CALC-STEP: temporary effective rate for LFO2 FM calculation.
      ...
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * /\    LFO USER INPUT TEMPS       /\
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * Plain (not COMP) so ACCEPT works correctly on all compilers.
       01  LFO-USER-INPUT.
           05  LFO-WAVE-IN     PIC 9(1).
           05  LFO-RATE-IN     PIC 9(3).
           05  LFO-DELAY-IN    PIC 9(3).
           05  LFO-FADE-IN     PIC S9(3).
           05  LFO-OFFSET-IN   PIC S9(3).
           05  LFO-TVA-D-IN    PIC S9(3).
           05  LFO-TVF-D-IN    PIC S9(3).
           05  LFO-PTCH-D-IN   PIC S9(3).
           05  LFO-PHASE-IN    PIC 9(3).
           05  LFO-KEYTRIG-IN  PIC 9(1).
           05  LFO-FM-D-IN     PIC S9(3).
      ...
      * /\      PROGRESS TRACKING         /\
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
       01  PROGRESS-VARS       USAGE IS COMP-5.
      ...
      * /\        WAVETABLE MEMORY        /\
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
       01  WAVE-GENERATOR-CONSTANTS USAGE IS COMP-5.
      ...
      * /\            RESAMPLE            /\
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
       01  RESAMPLE-POINTERS   USAGE IS COMP-5.
      ...
      * TVF (Filter Envelope) Variables
       ...
       01  TVF-CONTROL-VARS    USAGE IS COMP-5.
      ...
      * LUT coefficient smoother (one-pole IIR on the control signal)
      ...
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
       01  PCM-WRITING         USAGE IS COMP-5.
      ...
      * /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
      * Configure both LFOs, generate their wavetables, and
      * pre-calculate their runtime step sizes and state.
      ...
      * Logic from NOTE-SELECTOR.CPY determines which paragraph to run
       ...
       GET-INTERPOLATION-MODE.
           DISPLAY " ".
           DISPLAY "--- INTERPOLATION SEQUENCER ---".
           DISPLAY "KEY: 1=None, 2=Linear, 3=Sinc".
           DISPLAY "Enter Pattern (e.g. 3331, 1212): ".
           ACCEPT RAW-PATTERN-INPUT.
       ...
       PARSE-PATTERN.
           ...
           IF PATTERN-LENGTH = 0
               ...
               DISPLAY "Defaulting to Linear (2)."
           END-IF.
      ...
      * Pre-calc Bias Thresholds if needed
       ...
       GET-ENVELOPE-SETTINGS.
           DISPLAY " ".
           DISPLAY "--- JD-800 ENVELOPE (AMPLITUDE) ---".
      ...
      * Must run before TVF and TVA modulation paragraphs below.
      ...
      * Adds LFO signal to CURRENT-KNOB before coefficient lookup.
           PERFORM APPLY-LFO-TVF.
           PERFORM RECALCULATE-COEFFICIENTS.
           PERFORM APPLY-FILTER.
      * >>>> LFO TVA MODULATION (TREMOLO) <<<<
      * Scales INTERP-RESULT after filtering, before volume write.
      ...
      * ==========================================
      * RECALCULATE-COEFFICIENTS now uses the pre-built LUT for speed.
      * TVF-DEPTH is still the signed value entered by the user (-100..100).
      * ==========================================
      ...
      *      5. Direct Fetch from pre-built LUT (no slow SEARCH needed)
      ...
      * OPTIMIZED: INIT-SINC-TABLE now uses FUNCTION SIN/COS directly
      * instead of the lookup-table SEARCH approach. This is faster at
      * startup and removes the dependency on NORMALIZE-ANGLE during init.
       INIT-SINC-TABLE.
       ...
       NORMALIZE-ANGLE.
      * Kept for compatibility. Maps any angle to 0.0 - 6.283 range.
           COMPUTE ANGULAR-FREQUENCY = TEMP-ANGLE.
           PERFORM UNTIL ANGULAR-FREQUENCY >= 0
               ADD PI-2 TO ANGULAR-FREQUENCY
           END-PERFORM.
           PERFORM UNTIL ANGULAR-FREQUENCY < PI-2
               SUBTRACT PI-2 FROM ANGULAR-FREQUENCY
           END-PERFORM.
      ...
      * ==========================================
      * GENERATE-FILTER-LUT
      * Pre-calculates all 1001 biquad coefficient sets (one per 0.1 knob
      * step across the 0-100 range) at startup. During audio processing,
      * RECALCULATE-COEFFICIENTS does a direct array fetch instead of
      * running a SEARCH, saving the heavy trig work per sample.
      * ==========================================
       GENERATE-FILTER-LUT.
      ...
      * ==========================================
      * GET-LFO-SETTINGS
      * Interactive menu for both LFOs. Mirrors the JD800 panel
      * controls (Rate/Delay/Fade/Waveform/Offset/Key-Trig) and
      * adds: Pitch depth, Phase offset, and FM LFO1->LFO2.
      * ==========================================
       GET-LFO-SETTINGS.
           DISPLAY " ".
           DISPLAY "--- LFO 1 CONFIGURATION ---".
           DISPLAY "Waveform Types:".
           DISPLAY "  1=Sine      2=Triangle  3=Saw-Up".
           DISPLAY "  4=Saw-Down  5=Square    6=Sample+Hold".
           DISPLAY "  7=Smooth-Random  8=User RAW (LFO-Wave.raw)".
           DISPLAY "LFO1 Waveform (1-8): ".
           ACCEPT LFO-WAVE-IN.
           IF LFO-WAVE-IN < 1 OR LFO-WAVE-IN > 8
               MOVE 1 TO LFO-WAVE-IN
           END-IF.
           MOVE LFO-WAVE-IN TO LFO1-WAVEFORM.

           DISPLAY "LFO1 Rate Hz (0-100, decimals ok e.g. 3): ".
           ACCEPT LFO-RATE-IN.
           COMPUTE LFO1-RATE-HZ = LFO-RATE-IN.

           DISPLAY "LFO1 Delay Seconds (0-99 whole seconds): ".
           ACCEPT LFO-DELAY-IN.
           COMPUTE LFO1-DELAY-SEC = LFO-DELAY-IN.

           DISPLAY "LFO1 Fade Sec (+pos=FadeIn -neg=FadeOut 0=None):".
           ACCEPT LFO-FADE-IN.
           MOVE LFO-FADE-IN TO LFO1-FADE-SEC.

           DISPLAY "LFO1 Offset (-100 to +100, 0=centered): ".
           ACCEPT LFO-OFFSET-IN.
           MOVE LFO-OFFSET-IN TO LFO1-OFFSET.

           DISPLAY "LFO1 Key Trig (1=On reset phase  0=Free run): ".
           ACCEPT LFO-KEYTRIG-IN.
           IF LFO-KEYTRIG-IN NOT = 0 AND LFO-KEYTRIG-IN NOT = 1
               MOVE 1 TO LFO-KEYTRIG-IN
           END-IF.
           MOVE LFO-KEYTRIG-IN TO LFO1-KEY-TRIG.

           DISPLAY "LFO1 TVA Depth (-100 to +100 amplitude mod): ".
           ACCEPT LFO-TVA-D-IN.
           MOVE LFO-TVA-D-IN TO LFO1-TVA-DEPTH.

           DISPLAY "LFO1 TVF Depth (-100 to +100 cutoff mod): ".
           ACCEPT LFO-TVF-D-IN.
           MOVE LFO-TVF-D-IN TO LFO1-TVF-DEPTH.

           DISPLAY "LFO1 Pitch Depth (-120 to +120, 10=1 semitone):".
           ACCEPT LFO-PTCH-D-IN.
           MOVE LFO-PTCH-D-IN TO LFO1-PTCH-DPTH.

           DISPLAY "LFO1 Phase Offset (0-359 degrees): ".
           ACCEPT LFO-PHASE-IN.
           IF LFO-PHASE-IN > 359 MOVE 0 TO LFO-PHASE-IN END-IF.
           MOVE LFO-PHASE-IN TO LFO1-PHASE-OFFS.

           DISPLAY "LFO1 FM Depth to LFO2 rate (-100 to +100): ".
           ACCEPT LFO-FM-D-IN.
           MOVE LFO-FM-D-IN TO LFO1-FM-DEPTH.

           DISPLAY " ".
           DISPLAY "--- LFO 2 CONFIGURATION ---".
           DISPLAY "LFO2 Waveform (1-8): ".
           ACCEPT LFO-WAVE-IN.
           IF LFO-WAVE-IN < 1 OR LFO-WAVE-IN > 8
               MOVE 1 TO LFO-WAVE-IN
           END-IF.
           MOVE LFO-WAVE-IN TO LFO2-WAVEFORM.

           DISPLAY "LFO2 Rate Hz (0-100): ".
           ACCEPT LFO-RATE-IN.
           COMPUTE LFO2-RATE-HZ = LFO-RATE-IN.

           DISPLAY "LFO2 Delay Seconds (0-99): ".
           ACCEPT LFO-DELAY-IN.
           COMPUTE LFO2-DELAY-SEC = LFO-DELAY-IN.

           DISPLAY "LFO2 Fade (+FadeIn -FadeOut 0=None): ".
           ACCEPT LFO-FADE-IN.
           MOVE LFO-FADE-IN TO LFO2-FADE-SEC.

           DISPLAY "LFO2 Offset (-100 to +100): ".
           ACCEPT LFO-OFFSET-IN.
           MOVE LFO-OFFSET-IN TO LFO2-OFFSET.

           DISPLAY "LFO2 Key Trig (1=On  0=Off): ".
           ACCEPT LFO-KEYTRIG-IN.
           IF LFO-KEYTRIG-IN NOT = 0 AND LFO-KEYTRIG-IN NOT = 1
               MOVE 1 TO LFO-KEYTRIG-IN
           END-IF.
           MOVE LFO-KEYTRIG-IN TO LFO2-KEY-TRIG.

           DISPLAY "LFO2 TVA Depth (-100 to +100): ".
           ACCEPT LFO-TVA-D-IN.
           MOVE LFO-TVA-D-IN TO LFO2-TVA-DEPTH.

           DISPLAY "LFO2 TVF Depth (-100 to +100): ".
           ACCEPT LFO-TVF-D-IN.
           MOVE LFO-TVF-D-IN TO LFO2-TVF-DEPTH.

           DISPLAY "LFO2 Pitch Depth (-120 to +120): ".
           ACCEPT LFO-PTCH-D-IN.
           MOVE LFO-PTCH-D-IN TO LFO2-PTCH-DPTH.

           DISPLAY "LFO2 Phase Offset (0-359 degrees): ".
           ACCEPT LFO-PHASE-IN.
           IF LFO-PHASE-IN > 359 MOVE 0 TO LFO-PHASE-IN END-IF.
           MOVE LFO-PHASE-IN TO LFO2-PHASE-OFFS.

      * ==========================================
      * INIT-LFO-WAVEFORMS
      * Top-level controller. Dispatches to waveform-specific
      * builders, then initialises both runtime state blocks.
      * ==========================================
       INIT-LFO-WAVEFORMS.
           DISPLAY "Building LFO wavetables...".
           PERFORM INIT-LFO1-TABLE.
           PERFORM INIT-LFO2-TABLE.
           PERFORM INIT-LFO1-STATE.
           PERFORM INIT-LFO2-STATE.
           DISPLAY "LFO Engine Ready.".
      ...
      * ==========================================
      * INIT-LFO1-STATE
      * Calculates STEP, DELAY/FADE sample counts, and sets
      * initial phase. Called once at startup.
      * ==========================================
       INIT-LFO1-STATE.
           IF LFO1-RATE-HZ = 0
               MOVE 0 TO LFO1-STEP
           ...
           EVALUATE TRUE
               ...
               WHEN LFO1-FADE-SEC > 0
      * Positive fade seconds = fade in from silence to full
                   ...
                   COMPUTE LFO1-FADE-STEP =
                       1.0 / LFO1-FADE-TOTAL
               WHEN OTHER
      * Negative fade seconds = fade out from full to silence
                   MOVE 1.0 TO LFO1-FADE-AMT
                   COMPUTE LFO1-FADE-TOTAL =
      ...
      * LFO1 FM: if LFO1-FM-DEPTH is non-zero, LFO1-VALUE is used to
      * scale LFO2's effective step size, giving FM-style rate wobble.
      * ==========================================
       ADVANCE-LFO-ENGINES.
           PERFORM ADVANCE-LFO1.
           PERFORM ADVANCE-LFO2.
      * Sum LFO1 + LFO2 contributions per destination
           COMPUTE LFO-TVA-OUT =
      ...
      * ==========================================
      * ADVANCE-LFO1
      * Core per-sample oscillator for LFO1.
      * Handles: delay countdown, fade ramp, phase advance,
      * table lookup with linear interpolation, and offset clamp.
      * S+H mode: samples the table once per SH-PERIOD and holds.
      * ==========================================
       ADVANCE-LFO1.
           IF LFO1-STEP = 0
               ...
               EXIT PARAGRAPH
           END-IF.
      * 1. Delay period - output is silent until delay expires
           IF LFO1-DELAY-CNT > 0
      ...
      * 2. Advance fade envelope
           IF LFO1-FADE-TOTAL > 0 AND
              LFO1-FADE-CNT < LFO1-FADE-TOTAL
               ...
               END-IF
           END-IF.
      * 3. Advance phase by STEP, wrap within 0 to 2047
           ADD LFO1-STEP TO LFO1-PHASE.
           PERFORM UNTIL LFO1-PHASE < 2048.0
               SUBTRACT 2048.0 FROM LFO1-PHASE
           END-PERFORM.
      * 4a. S+H mode: only update held value once per period
           IF LFO1-SH
           ...
           ELSE
      * 4b. All other modes: interpolated table read
           ...
           END-IF.
      * 5. Apply DC offset (OFFSET shifts -100..+100 / 100 = +/-1.0)
           ...
           IF LFO-WORK-VAL < -1.0 MOVE -1.0 TO LFO-WORK-VAL END-IF.
      * 6. Multiply by fade amplitude
           COMPUTE LFO1-VALUE = LFO-WORK-VAL * LFO1-FADE-AMT.

      * ==========================================
      * ADVANCE-LFO2
      * Identical to LFO1 except:
      *   - Uses LFO2-STATE and LFO2-PARAMS
      *   - Before phase advance, FM modulation from LFO1 is applied
      *     to the effective step size (LFO-CALC-STEP).
       ...
       ADVANCE-LFO2.
           ...
           IF LFO2-FADE-TOTAL > 0 AND
              LFO2-FADE-CNT < LFO2-FADE-TOTAL
               ...
               END-IF
           END-IF.
      * FM: modulate LFO2 step by LFO1 output
      ...
      * ==========================================
      * APPLY-LFO-TVF
      * Adds the combined LFO TVF signal to CURRENT-KNOB.
      * CURRENT-KNOB is the running envelope cutoff value.
      * RECALCULATE-COEFFICIENTS will clamp DEPTH-CALC to 0-100
      * after adding TVF-DEPTH scaling, so no clamping needed here.
      * LFO-TVF-OUT is in raw knob units (-200 to +200 theoretical).
      * ==========================================
       APPLY-LFO-TVF.
      ...
      * ==========================================
      * APPLY-LFO-TVA
      * Multiplies INTERP-RESULT (floating point -1.0..+1.0) by a
      * depth-scaled LFO signal to produce amplitude modulation.
      * Formula: output = input * (1.0 + LFO-TVA-OUT)
      * LFO-TVA-OUT of 0.0 = no change. +/-1.0 = full range.
      ...
      * ==========================================
      * APPLY-LFO-PITCH
      * Adds a fractional READ-POSITION offset based on the LFO
      * pitch signal to produce vibrato.
      * Formula uses semitone-ratio approximation:
      *   semitone_ratio = 2^(1/12) - 1 ~= 0.059463094
      *   delta = STEP-SIZE * semitones * 0.059463094
      * LFO-PTCH-OUT is in semitones (1.0 = 1 semitone).
      * Called before COMPUTE-RAW-SAMPLE so CALCULATE-INDICES
      * has already captured READ-INDEX for this sample.
      * ==========================================
       APPLY-LFO-PITCH.
      ...
      * Each fills the 2048-entry LFO1-WAVE-TABLE with values in
      * the range -1.0 to +1.0.
      * ==========================================

      * --- SINE WAVE: Uses FUNCTION SIN for maximum accuracy. ---
       GEN-SINE-TO-LFO1.
      ...
      * --- TRIANGLE: Linear ramp 0->1, then 1->-1, then -1->0 ---
       GEN-TRI-TO-LFO1.
           PERFORM VARYING LFO-BUILD-IDX FROM 1 BY 1
               UNTIL LFO-BUILD-IDX > 2048
               COMPUTE LFO-BUILD-ANG =
      ...
      * --- SQUARE: First 1024 entries = +1, remainder = -1 ---
       GEN-SQUARE-TO-LFO1.
           PERFORM VARYING LFO-BUILD-IDX FROM 1 BY 1
               UNTIL LFO-BUILD-IDX > 2048
      ...
      * --- SAMPLE+HOLD: 16 random steps, each 128 entries wide ---
      * Pre-fills table so the core ADVANCE loop needs no branching.
       GEN-SH-TO-LFO1.
      ...
      * --- SMOOTH RANDOM: 16 random targets, linearly interpolated --
      * Generates a stepped random sequence then blends between steps.
       GEN-SMTH-RND-TO-LFO1.
           ...
           PERFORM VARYING LFO-BUILD-IDX FROM 1 BY 1
               UNTIL LFO-BUILD-IDX > 2048
      * Calculate position within the current 128-sample step
               COMPUTE LFO-FRAC =
                   FUNCTION MOD(LFO-BUILD-IDX - 1, 128) / 128.0
      * At the start of each new step, pick the next random target
      ...
      * Reads 2048 samples (4096 bytes) in little-endian order.
      * If the file is shorter the remaining entries are zeroed.
       LOAD-LFO-RAW-TO-LFO1.
           DISPLAY "Loading LFO1 user waveform from LFO-Wave.raw...".
           OPEN INPUT LFO-FILE.
           PERFORM VARYING LFO-BUILD-IDX FROM 1 BY 1
               UNTIL LFO-BUILD-IDX > 2048
               ...
               READ LFO-FILE INTO LFO-FILE-BUF(1:1)
                   AT END
                       DISPLAY "LFO RAW file ended early - zeroing."
                       EXIT PERFORM
                   NOT AT END
                       READ LFO-FILE INTO LFO-FILE-BUF(2:1)
                           AT END
                               DISPLAY "LFO RAW odd byte count."
                               EXIT PERFORM
                           NOT AT END
                               COMPUTE LFO-FILE-W1 =
                                 FUNCTION ORD(LFO-FILE-BUF(1:1)) - 1
                               COMPUTE LFO-FILE-W2 =
                                 FUNCTION ORD(LFO-FILE-BUF(2:1)) - 1
                               COMPUTE LFO-FILE-W1 =
                                 LFO-FILE-W1 + (LFO-FILE-W2 * 256)
                               IF LFO-FILE-W1 > 32767
                                   SUBTRACT 65536 FROM LFO-FILE-W1
                               END-IF
                               COMPUTE LFO1-TBL-SAMP(LFO-BUILD-IDX) =
                                   LFO-FILE-W1 / 32768.0
                       END-READ
           ...
           END-PERFORM.
           CLOSE LFO-FILE.
           DISPLAY "LFO1 User RAW waveform loaded.".

      * ==========================================
      * WAVEFORM GENERATORS - LFO2
      * Exact mirror of LFO1 generators, writing into LFO2-WAVE-TABLE.
      * USER RAW copies from the already-loaded LFO1 table since
      * both LFOs share the single LFO-Wave.raw source file.
      * ==========================================
      ...
      * LFO2 USER RAW: copy from the LFO1 table (same source file).
      * Avoids re-opening the file and re-reading. If LFO1 was set
      * to a different waveform type, this loads from LFO-Wave.raw
      * directly, same as LOAD-LFO-RAW-TO-LFO1.
       LOAD-LFO-RAW-TO-LFO2.
           IF LFO1-USER-RAW
      * LFO1 already loaded the file - just copy the table
               PERFORM VARYING LFO-BUILD-IDX FROM 1 BY 1
                   UNTIL LFO-BUILD-IDX > 2048
               ...
               END-PERFORM
           ELSE
      * LFO1 used a different waveform, load the raw file directly
               DISPLAY "Loading LFO2 user waveform from LFO-Wave.raw."
               OPEN INPUT LFO-FILE
               PERFORM VARYING LFO-BUILD-IDX FROM 1 BY 1
                   UNTIL LFO-BUILD-IDX > 2048
      ...
      * ==========================================
      * GEN-LFO-RANDOM
      * Park-Miller LCG pseudo-random number generator.
      * Writes a float 0.0 to 1.0 into LFO-RND-RESULT.
      * Identical algorithm to GENERATE-DRIFT in the analogue engine
      * but uses its own seed so the two engines are independent.
      * ==========================================
       GEN-LFO-RANDOM.
